...
- ⚙️ **돌봄 관리자 설정 모드**: [`realtime-voice/care-admin.html`](realtime-voice/care-admin.html) — 관리자가 음성 대화로 돌봄 AI의 말투·추임새·안내사항·맥락관리를 맞춤 설정 → `localStorage.care_config`에 저장 → care.html이 통화 시 주입(같은 Pages 도메인이라 공유). 텔레그램 **"돌봄 설정/관리자/통화 조정"** → care-admin.html 링크. **대상자(어르신) 관리**: 단말기 대상자가 바뀔 때 ① **음성 초기화**(학습된 목소리 패턴만 삭제→다음 통화 재학습) ② **전체 초기화**(목소리+로컬 기억+서버 기억[care_api `/care-reset`로 `care_profiles.memo`/visits 리셋]) 버튼으로 새 대상자 전환
...
- 🩺 **통합 관리자 모니터링 대시보드**: [`realtime-voice/admin.html`](realtime-voice/admin.html) — 모바일·PC 반응형. **단말기별 접속상태(🟢통화중/⚪오프라인)·승인상태(✅승인/⏳대기/⛔차단)·접속횟수·통화시간·토큰사용량**을 카드로, 최근 통화 로그를 표로 보여주고, 카드를 누르면 그 단말기의 **누적 기억(메모) + 통화별 대화기록**을 상세히 확인. 관리자 패스코드로 보호(`admin_api` 워크플로우 `인증` 코드 노드에서 설정·변경). 데이터: care.html이 통화 중 핑(`/care-ping`, 온라인표시)·종료 시 세션로그(`/care-session`)를 서버에 적재 → `admin_api`의 `/admin-data`가 집계. 텔레그램 **"모니터링/대시보드/접속현황"** → admin.html 링크. ⚠️ 토큰사용량은 Realtime `response.done`의 usage를 합산한 근사치. **💲 비용**: 요청당 비용 = 입력토큰 비용 + 출력토큰 비용(요율은 admin.html `COST` 상수에서 수정, 로컬 LLM 전환 시 0으로). 대시보드 "예상 비용"(USD+₩) + 사용 분석 지표/단말기별 표에 표시. **📊 사용 분석 탭**: 기간(일/주/월/분기/년)·지표(통화수/통화시간/토큰/비용)·단말기·지역 필터로 **다차원 분석** — 기간별 추이·지역별·단말기별 막대그래프(단말기 행 클릭 시 그 단말기로 드릴다운)
- 🧠 **LLM 선택(클라우드/로컬 하이브리드)**: 관리자 대시보드 "🧠 LLM 선택"에서 OpenAI(클라우드) 또는 Ollama(로컬) 모델 선택. **로컬 LLM 선택 시 자동 하이브리드** — 실시간 통화는 OpenAI Realtime(`gpt-realtime`) 유지(4GB GPU로는 로컬 실시간 불가), **통화 후 기억 요약(`care-summary`) 등 텍스트 작업은 로컬 LLM 사용**. 날씨/뉴스 등 외부정보는 통화 중 도구 호출이므로 클라우드 LLM이 처리. 추천 한국어 친화 소형 모델: `exaone3.5:2.4b`(LG, 한국어 특화 ~1.6GB), `qwen2.5:3b`(다국어 ~1.9GB), `gemma2:2b`. 설치: Ollama 설치 후 `ollama pull exaone3.5:2.4b`. `llm_config` 테이블+`/admin-llm`·`/admin-llm-set` 엔드포인트. **n8n→Ollama URL은 `127.0.0.1`**(localhost는 IPv6로 잡혀 ECONNREFUSED)
- 🖥️ **서버/LLM 부하 모니터링**: admin.html 상단 카드에 로컬 서버 **CPU·메모리·디스크·GPU 사용율**(게이지) + **LLM 활동**(동시 통화수·최근 1시간 통화/토큰) 표시. `tools/metrics-agent.cjs`(Node, 무의존성)가 10초마다 지표를 `/sys-push`로 전송→`sys_metrics` 저장→`/admin-sys`로 조회. **GPU는 nvidia-smi 자동 감지**(향후 로컬 LLM GPU 부하 모니터링 대비). 수집기 미실행 시 LLM 활동만 표시. 실행: `node tools/metrics-agent.cjs` (부팅 자동실행은 작업 스케줄러/pm2)
- 🛂 **신규 단말기 승인제(허가 없는 URL 사용 차단)**: care.html은 단말기 `status`(pending/approved/blocked)로 관리. **새 단말기가 처음 접속하면 텔레그램으로 관리자에게 알림**(care_api `/care-profile-save`가 신규 INSERT 감지→Telegram), 관리자가 대시보드 카드의 **[✅승인]/[⛔차단(사유 입력)]** 버튼으로 결정(`/care-block`, 패스코드 보호). 대시보드에서 **체크박스로 단말기 선택 삭제·전체 삭제**도 가능(`/care-delete`). **승인 전(pending)·차단(blocked) 단말기는 `/realtime-token`이 토큰 발급을 거부**해 통화 불가(서버측 강제라 클라이언트 우회 불가). care.html은 "승인 대기 중"/"차단됨" 안내 표시. (기존 사용 중이던 단말기는 마이그레이션 시 approved로 보존)
...
| `workflows/care_api.json` | care.html 클라이언트 API. `POST /realtime-token`→**status=approved일 때만** OpenAI 단기토큰(ek_) 발급(pending/blocked 거부), `POST /care-summary`→기억 요약(device_id 기준 memo 저장), `GET /care-profile`·`POST /care-profile-save`(신규면 Telegram 알림), `POST /care-session`→통화 로그, `POST /care-ping`→온라인 핑, `POST /care-block`→승인/차단 결정(+차단사유, 패스코드), `POST /care-reset`→대상자 기억 초기화, `POST /care-delete`→단말기 선택/전체 삭제(패스코드). OpenAI/Postgres/Telegram 자격증명·CORS |
| `workflows/admin_api.json` | 관리자 대시보드 API. `GET /admin-data?pass=`→단말기 집계+통화로그, `GET /admin-analytics?pass=&period=&device_id=&region=`→**다차원 사용 분석**(기간[day/week/month/quarter/year]·지역·단말기 필터, json_agg 단일쿼리로 byPeriod/byRegion/byDevice 반환). admin.html이 호출. **패스코드는 `인증`/`분석빌드` 코드 노드에서 변경**(export엔 `CHANGE_ME_ADMIN_PASS` 플레이스홀더) |
