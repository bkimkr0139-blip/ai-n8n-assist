...
| `workflows/care_api.json` | 키 없이 외부 사용용 API. `POST /realtime-token`→OpenAI 단기토큰(ek_) 발급, `POST /care-summary`→통화 기억 요약. 둘 다 n8n의 OpenAI 자격증명 사용·CORS 허용. care.html이 호출(브라우저에 키 노출 안 함) |
