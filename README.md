...
| `workflows/care_api.json` | care.html 클라이언트 API. `POST /realtime-token`→**status=approved일 때만** OpenAI 단기토큰(ek_) 발급(pending/blocked 거부), `POST /care-summary`→기억 요약(device_id 기준 memo 저장), `GET /care-profile`·`POST /care-profile-save`(신규면 Telegram 알림), `POST /care-session`→통화 로그, `POST /care-ping`→온라인 핑, `POST /care-block`→승인/차단 결정(패스코드), `POST /care-reset`→대상자 기억 초기화. OpenAI/Postgres/Telegram 자격증명·CORS |
